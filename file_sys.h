// $Id: file_sys.h,v 1.6 2016/07/13 00:10:55 mchaboll Exp $
//Marcos Chabolla
//ID:1437530
...
class inode_state {
   ...
   public:
      ...
      inode_ptr get_root();
      inode_ptr get_cwd();
      inode_ptr return_parent_path(const string& path);
      inode_ptr return_inode_path(const string &);
      void set_cwd(inode_ptr);
      string get_prompt() const;
      void set_prompt(string);
      bool recursive_free(inode_ptr);
};
...
class inode {
   ...
   public:
      ...
      file_type get_type();
      void print_dir(stringstream & stream) const;
      void print_desc(stringstream &, string = "") const;
      void recursive_print(stringstream & ss,
      deque<dirent_pair> &, string) const;
     
      // Seg faults on return size
};
...
class base_file {
   ...
   public:
      ...
      virtual inode_ptr mkdir (const string& dirname, inode_ptr pointer) = 0;
...
class plain_file: public base_file {
   ...
   public:
      ...
      virtual inode_ptr mkdir (const string& dirname, inode_ptr pointer) override;
...
class directory: public base_file {
   ...
   public:
      ...
      virtual inode_ptr mkdir (const string& dirname, inode_ptr pointer) override;
      virtual inode_ptr mkfile (const string& filename) override;
      void initialize(inode_ptr, inode_ptr);
      inode_ptr get_dirents(string);
      vector<dirent_pair> convert_to_vector() const;
